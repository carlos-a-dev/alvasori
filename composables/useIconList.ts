...
function levenshtein(a: string, b: string): number {
  ...
  let prevRow = Array(bLen + 1).fill(0).map((_, i) => i)
...
// Main function to filter array based on regex and Levenshtein distance
function filterArray(array: string[], searchTerm: string, maxResults: number): string[] {
  if (searchTerm === '') {
  ...
  // Length check optimization (optional)
  const searchTermLength = searchTerm.length

  // Use filter method to apply the regex to each element in the array
  const filteredArray = array.filter(item => item.length >= searchTermLength && regex.test(item))

  const array2sort = filteredArray.length > 0 ? filteredArray : array

  const distances = array2sort.map(item => ({ item, distance: levenshtein(item, searchTerm) }))

  // Sort by distance
  distances.sort((a, b) => a.distance - b.distance)

  // Return the top closest matches (up to maxResults)
  return distances.slice(0, maxResults).map(d => d.item)
}

function searchIcons(search: string, limit: number = 20) {
  return filterArray(iconList, search.toLowerCase().trim().replace('mdi-', '').replace('mdi', ''), limit)
}
